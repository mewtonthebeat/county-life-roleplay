...
Character::CreateNew(playerid)
{
	...
	if(Character::count[playerid] >= 2)
	{
	    ...
	    {
	        SendError(playerid, "Pokiaæ si chceö zaloûiù ÔalöÌ charakter, musÌö maù aspoÚ 300 mincÌ.");
	        return 1;
		}
		...
		format(string, 144, "{ffffff}> Pokiaæ si chceö zaloûiù Ôalöiu postavu, musÌö zaplatiù poplatok %d mincÌ.\n> SkutoËne chceö vytvoriù t˙to postavu a zaplatiù poplatok?", MULTIPLE_CHARACTER_FEE);
...
CharacterCreate::Create(playerid)
{
    ...
    SendSuccess(playerid, "Postava bola vytvoren·, mÙûeö si ju vybraù!");
...
CharacterCreate::UpdateSkin(playerid, bool:next = false, skinid = -1)
{
	if(skinid != -1)
	{
		...
		if(forgender == -1)
		{
		    SendError(playerid, "Skin s t˝mto ID neexistuje, alebo ho nemÙûeö pouûÌvaù!");
		    return 1;
		}
		
		if(forgender != CharacterCreate::gender[playerid])
		{
		    SendError(playerid, "Skin s t˝mto ID nie je pre nastavenÈ pohlavie!");
		    return 1;
		}
...
{
    ShowPlayerDialog(playerid, did_char_cre_name, DIALOG_STYLE_INPUT, "MENO POSTAVY", "{FFFFFF}> Zadaj prosÌm meno postavy v tvare {e34f4f}Meno_Priezvisko{FFFFFF}.", "OK", "SPAT");
    Character::noresponse[playerid] = true;
	return 1;
}
...
Character::OnPlayerClickPTD(playerid, PlayerText:playertextid)
{
	if(Character::isincreation[playerid] && Character::noresponse[playerid] == false)
	{
	    ...
	    {
	        // manualne ID skinu
	        ShowPlayerDialog(playerid, did_char_create_skin, DIALOG_STYLE_INPUT, "MANU¡LNY V›BER SKINU", "{FFFFFF}> Pokiaæ vieö ID skinu, ktor˝ chceö, zadaj ho sem!", "NASTAVIç", "ZRUäIç");
	        Character::noresponse[playerid] = true;
	    }
	    ...
	    else if(playertextid == character_CreateTD[playerid][8])
	    {
	        ...
	        if(strlen(CharacterCreate::name[playerid]) > 5)
			    ...
			    SendError(playerid, "Najprv si zvoæ meno charakteru!");
	    }
	}
